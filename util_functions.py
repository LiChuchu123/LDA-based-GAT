...
class MyDataset(InMemoryDataset):
    ...
    def process(self):
        # Read row_data into huge `Data` list.
...
def subgraph_extraction_labeling(ind, A, h=1, u_features=None, v_features=None, class_values=None):
    ...
    for dist in range(1, h+1):
        ...
        if len(u_fringe) == 0 and len(v_fringe) == 0:
            break
        u_nodes = list(u_nodes) + list(u_fringe)
        v_nodes = list(v_nodes) + list(v_fringe)
...
def extracting_subgraphs(
    ...
    def helper(A, links, g_labels):
        ...
        #
        #     list = [(parallel_worker(g_label,(i,j),A,h,u_features,v_features,class_values))]
        #     results+=list
        ...
        #     g_list += [nx_to_PyGGraph(g, g_label, n_labels, n_features, max_node_label, class_values)]
